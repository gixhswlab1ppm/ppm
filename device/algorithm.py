...
def triangulate(data):
    ...
    for datum in data:
        ...
        if (DistA != 0) and (DistB != 0) and (DistC != 0):
            ...
            print("Piezometer 1: ", DistA, ", 2: ", DistB,
                  ", 3: ", DistC, ", period: ", i, guess, sep='')
        i += 1
...
#     fft_topk = 20
#     # 2D (feature, window) array of (list of) amps
#     fft_amps = np.ndarray((n_feature, n_window, len(fft_freqs)))
...
#             m_i for m_i in maxima_idx if data[m_i, i+1] > threshold]
#         ts_dedup = clustering_dedup(data[maxima_idx_filtered, 0])
#         print(i, "-th feature ", 'detected ', len(ts_dedup), ' peaks')

#         if visualize == True:
#             # plt.figure(str(i) + '-th data')
#             plt.scatter(data[maxima_idx_filtered, 0], [
#                         1-0.1*i]*len(maxima_idx_filtered), s=10*data[maxima_idx_filtered, i+1]/threshold)
#             plt.scatter([np.mean(seg) for seg in ts_dedup], [
#                         1.5+0.05*i]*len(ts_dedup), s=[10*len(seg)for seg in ts_dedup], alpha=.5)
#             if i == 2:
...
#         if np.sum(mpu[i, [7, 8, 9]]) > 0:
#             pass
#             # print(triangulate_centroid(mpu[i, [7, 8, 9]]))
#     try:
#         period_pred = np.max([val for val, count in Counter(
#             [p for p, a in period_poll]).most_common(1)])
